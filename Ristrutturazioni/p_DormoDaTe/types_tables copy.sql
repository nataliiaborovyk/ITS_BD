...
create table chiusurastraordinaria(
    id serial primary key,
    data_inizio date not null,
    data_fine date not null,
    motivo stringa,
    check (data_inizio < data_fine)
    -- v.inclusione chiusurastraordinaria(id) occorre in al_cs(chiusura_straordinaria_id)
);

create table al_cs(
    alloggio_id integer not null,
    chiusura_straordinaria_id integer not null,
    primary key (alloggio_id, chiusura_straordinaria_id),
    foreign key (alloggio_id)
        references alloggio(id),
    foreign key (chiusura_straordinaria_id)
        references chiusurastraordinaria(id)
);
